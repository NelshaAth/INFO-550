...
raw_unit_96$Date <- str_sub(raw_unit_96$timestamp, 1, 10)
  head(raw_unit_96$Date) # Date only
...
ggplot(raw_unit_96, aes(x=Lubri_Date)) + 
  ...
  ggtitle("Unit 96 AOD Values by Wavelength - September 2021") +
            xlab("Time") + ylab("AOD")
```
Unit 96 has wavelength channels that are crossing over causing incorrect AOD measurements



```{r Graph 6}
three_day_96_Subset <- subset(raw_unit_96, Date == c("2021-09-01", "2021-09-02", "2021-09-03", "2021-09-04"))
dim(three_day_96_Subset)

ggplot () +
  geom_point(data=three_day_96_Subset[!is.na(three_day_96_Subset$plantower_pm25),], 
            aes(x = Lubri_Date, y = plantower_pm25), color="darkred") +
  geom_line(data=three_day_96_Subset[!is.na(three_day_96_Subset$plantower_pm25),], 
            aes(x = Lubri_Date, y = plantower_pm25), color="red") +
  geom_point(data=raw_unit_91[!is.na(raw_unit_91$plantower_pm25),], 
            aes(x = Lubri_Date, y = plantower_pm25), color="darkblue") +
  geom_line(data=raw_unit_91[!is.na(raw_unit_91$plantower_pm25),], 
            aes(x = Lubri_Date, y = plantower_pm25), color="blue")
```
...
```{r Graph 7}
Day_Match_91 <- subset(raw_unit_91, Date == c("2021-09-01", "2021-09-02", "2021-09-03", "2021-09-04") & aod_675 > 0)
  dim(Day_Match_91)
Day_Match_96 <- subset(raw_unit_96, Date == c("2021-09-01", "2021-09-02", "2021-09-03", "2021-09-04") & aod_675 > 0)
  dim(Day_Match_96)
  
comb_Subs_91_96 <- rbind(Day_Match_91, Day_Match_96)
  dim(comb_Subs_91_96)

ggplot(data=comb_Subs_91_96, aes(x=aod_675, y=aod_675, group="dev_sn")) +
  geom_point(aes(colour = factor(dev_sn)), size = 3) + 
  geom_smooth(method='lm', size = 1)

ggplot(data=comb_Subs_91_96, aes(x=aod_675, y=aod_675, group="dev_sn")) +
  geom_point(aes(colour = factor(dev_sn)), size = 3) +
  geom_smooth(method='lm',formula=comb_Subs_91_96$y.plot~data$x.plot)
  
```

AOD values between units have zero agreement. Unit 96 needs to be realigned.
